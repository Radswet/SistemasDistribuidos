...
async function insertReceta(newP_id,comentario,farmacos,doctor){
    const query = `SELECT MAX(id) AS max_id FROM ks2.recetas;`;
    const Recetas = await cassandra_client.execute(query);
    const newR_id = Recetas.rows[0].max_id + 1;
    const queryInsert = `INSERT INTO ks2.recetas(id, id_paciente, comentario, farmacos, doctor)
  ...
  app.post("/create", async (req, res) => {
    ...
    if(data.rowLength==0){
     // Es como un if not exist
      const newP_id = await insertPaciente(nombre, apellido, rut, email, fecha_nacimiento);
...
app.post("/edit", async (res, req) => {
    ...
    if(data.rowLength==0){
      ...
      const query= `UPDATE ks2.recetas SET comentario = '${comentario}', farmacos = '${farmacos}', doctor = '${doctor}' WHERE id = ${id};`;
